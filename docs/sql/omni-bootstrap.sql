-- Omnivia — Supabase Bootstrap SQL
-- Purpose: Create core schema, indexes, RLS helpers and policies, and Storage policies.
-- Usage: Paste into Supabase SQL Editor and run once (re-runnable for policies via DROP IF EXISTS).
-- Safe to run before/after creating the Storage bucket. If the bucket doesn't exist yet,
-- re-run the Storage Policy section after creating the bucket `attachments`.
