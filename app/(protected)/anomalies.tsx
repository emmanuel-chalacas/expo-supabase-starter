...
const SEVERITY_ORDER: Array<AnomalyRow["severity"]> = ["error", "warning", "info"];
...
export default function AnomaliesScreen() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const { data: rowsData, error: rowsErr } = await supabase.rpc("fn_anomalies_for_operator", {
        p_window_hours: 24,
        p_tenant: null,
        p_severity: null,
      });
      ...
      const { data: statsData, error: statsErr } = await supabase.rpc("fn_anomalies_stats", {
        p_window_hours: 24,
      });
      if (statsErr) {
  ...
  const grouped = useMemo(() => {
    const g: Record<AnomalyRow["severity"], AnomalyRow[]> = { error: [], warning: [], info: [] };
    for (const r of rows) {
  ...
  const statsTotals = useMemo(() => {
    let total = 0;
    const bySeverity: Record<AnomalyRow["severity"], number> = { error: 0, warning: 0, info: 0 };
  ...
  return (
    <ScrollView className="flex-1 bg-background p-4">
      <View className="mb-4 flex-row items-center justify-between">
        <H2>Anomalies</H2>
        <Button variant="default" size="sm" onPress={() => void refresh()} disabled={loading}>
      ...
      <View className="mb-4">
        ...
        <View className="mt-2 rounded-md border border-border p-3">
          ...
          <View className="flex-row gap-x-4">
            <Muted>
              error: <Text className="text-destructive font-medium">{statsTotals.bySeverity.error}</Text>
            </Muted>
            <Muted>
              warning: <Text className="text-yellow-600 font-medium">{statsTotals.bySeverity.warning}</Text>
            </Muted>
            <Muted>
              info: <Text className="text-foreground font-medium">{statsTotals.bySeverity.info}</Text>
            </Muted>
          </View>
        </View>
      </View>
            ...
            <View key={sev} className="mb-6">
              ...
              <View className="rounded-md border border-border">
                {list.map((r, idx) => (
                  <View
                    key={r.id}
                    className={["p-3", idx < list.length - 1 ? "border-b border-border" : ""].join(" ")}
                  >
                    ...
                    <P className="mb-1">
                      <Text className="font-semibold">{r.anomaly_type}</Text>{" "}
                      <Muted>tenant</Muted>{" "}
                      ...
                      {r.project_key ? (
                        <>
                          <Muted>project</Muted>{" "}
                          <Text className="font-medium">{r.project_key}</Text>{" "}
                        </>
                      ) : null}
                    </P>
      ...
      {!errMsg && rows.length === 0 ? (
        <Muted className="mt-8 text-center">No anomalies in the last 24 hours.</Muted>
      ) : null}

      {/* References and anchors for audit */}
      <Muted className="mt-8">
        RPC: fn_anomalies_for_operator / fn_anomalies_stats; see{" "}
        <Text className="underline">[sql.vw_import_anomalies_recent](supabase/migrations/20250818024500_stage5_anomalies_views_policies.sql:1)</Text>
      </Muted>
    </ScrollView>
  );
}