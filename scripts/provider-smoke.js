...
async function main() {
  ...
  const provider = String(env.EXPO_PUBLIC_SUPABASE_OIDC_PROVIDER || '').trim() || null;
  const enableAuth = String(env.EXPO_PUBLIC_ENABLE_OKTA_AUTH || '').trim().toLowerCase() === 'true';
  const issuerRaw = String(env.EXPO_PUBLIC_OKTA_ISSUER || '').trim();
  const endSessionRedirectRaw = String(env.EXPO_PUBLIC_OKTA_END_SESSION_REDIRECT || '').trim();
  ...
  if (!endSessionRedirectRaw) {
    summary.warnings.push('missing EXPO_PUBLIC_OKTA_END_SESSION_REDIRECT — using default omnivia://signout');
  }
