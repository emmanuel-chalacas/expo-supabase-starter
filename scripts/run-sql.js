...
function usageAndExit() {
	console.log(
		'Usage: node scripts/run-sql.js [<path-to-sql>] [-e "<inline-sql>"] [--remote]',
	);
	console.log("Notes:");
	console.log(
		"  - Provide either a SQL file OR -e/--execute (mutually exclusive).",
	);
	console.log(
		"  - If neither is provided, defaults to docs/sql/stage2-verify.sql.",
	);
	console.log(
		"  - --remote is a compatibility flag; connection is determined by env (SUPABASE_DB_URL or project_id+SUPABASE_DB_PASSWORD).",
	);
	console.log("Connection resolution (priority):");
	console.log(
		"  1) SUPABASE_DB_URL (sslmode=require will be appended if missing)",
	);
	console.log(
		"  2) project_id from supabase/config.toml + SUPABASE_DB_PASSWORD",
	);
	process.exit(2);
}

function readProjectIdFromToml(
	tomlPath = path.join(process.cwd(), "supabase", "config.toml"),
) {
	try {
...
function resolveConnectionString() {
	...
	if (!projectId) {
		fail(
			'Missing project_id in supabase/config.toml. Link the repo with "yarn supa:link" or provide SUPABASE_DB_URL.',
		);
	}
	const password = (process.env.SUPABASE_DB_PASSWORD || "").trim();
	if (!password) {
		fail(
			"Missing SUPABASE_DB_PASSWORD. Set it or provide SUPABASE_DB_URL instead. Do not commit secrets.",
		);
	}
...
function printTable(rows) {
	...
	for (const r of rows) {
		const line =
			"|" +
			cols
				.map((c, i) => " " + formatValue(r[c]).padEnd(widths[i]) + " ")
				.join("|") +
			"|";
		console.log(line);
	}
...
async function main() {
	...
	// Enable SSL for Supabase hosts to avoid self-signed cert chain issues in Node 'pg'
	const sslNeeded = /supabase\.co|supabase\.com/i.test(connectionString);
	...
	try {
		...
		if (rows.length > 0) {
			...
			const looksLikeVerify = Object.prototype.hasOwnProperty.call(
				rows[0] || {},
				"label",
			);
			if (looksLikeVerify) {
				...
				for (const row of rows) {
					const pass =
						row.pass === null || row.pass === undefined
							? ""
							: row.pass
								? "PASS"
								: "FAIL";
					console.log(
						`${row.label} | ${pass} | ${row.matches ?? ""} | ${row.rows ?? ""} | ${row.details ?? ""}`,
					);
				}
			} else {
				printTable(rows);
		...
		await client.end();
		console.log(
			`[run-sql] Completed in ${Math.round((Date.now() - started) / 1000)}s`,
		);
		process.exit(0);
	} catch (err) {
